...
module Parser =
    ...
    let xlerror: Parser<xlexpr, unit> =
        ...
        (ch '#') >>. regex "(N(\/A|AME\?|(ULL|UM)!)|(DIV\/0|VALUE|REF)!)"
        |>> errorStrToError
        |>> Err
        |>> Const
        |>> Type
    ...
    let xlref: Parser<xlexpr, unit> =
        optional (ch '$') >>. (many1 letter) .>> optional (ch '$') .>>. pint32
        |>> (fun (x, y) -> (lettersToInt x, y) |> Cell |> Ref |> Type)

    let xltext: Parser<xlexpr, unit> =
        between (ch '"') (ch '"') (many1 letter)
