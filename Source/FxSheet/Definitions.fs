    ...
    type DefinitionCompiler(compiler: ICompiler) =
        member x.Compile(d: Definition) =
            ...
            { Method = dynamicMethod
              ...
              ArgumentAttributes =
                d.ArgumentDescriptions
                |> List.map (fun x -> x :> obj)
                |> System.Linq.Enumerable.ToList }

    type DefinitionManager(pending: List<Definition>, registered: Map<string, Definition>, compiler: DefinitionCompiler) =
        ...
        member this.RegisterDefinitions() =
            ...
            let argumentAttributes =
                System.Collections.Generic.List<System.Collections.Generic.List<obj>>()
            ...
            uniquePending |> List.iter (fun d -> registered <- registered.Add(d.Name, d))
            let compiled = uniquePending |> List.map (compiler.Compile)
